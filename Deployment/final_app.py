...
def _save_uploaded_file(uploaded_file):
    """Save uploaded file to uploaded_files/ and return path and safe filename"""
    try:
...
def main():
    ...
    .stButton > button { background: linear-gradient(45deg, #e74c3c, #c0392b); color: white; border: none; border-radius: 25px; padding: 0.5rem 2rem; font-weight: bold; }
    .stFileUploader > div > div > div { background: white; border-radius: 10px; border: 2px dashed #e74c3c; }
    </style>
    """, unsafe_allow_html=True)
    ...
    # accept_multiple_files=True helps avoid axios 400 in some deployments
    ...
    # session keys to avoid repeated processing
    ...
    if uploaded_files:
        uploaded_file = uploaded_files[0]  # process first file
        try:
        ...
        if st.session_state['last_upload_hash'] == file_hash and st.session_state['last_upload_result'] is not None:
            ...
            try:
                ...
                saved_path, filename, _ = _save_uploaded_file(uploaded_file)
                progress_bar.progress(20)
    
                # clean up stale Scaled_ CSVs in ecg.base_dir (prevents repeat/picking old files)
                ...
                # show preview images only if they exist (use absolute paths)
                ...
                # Save prediction to MongoDB for auditing/tracking
                ...
                # Next steps & disclaimer (same as your original)
                st.markdown("""
